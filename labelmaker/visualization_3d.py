...
def main(args):
    mesh = read_mesh(args.workspace)  # 读取mesh
    os.makedirs(os.path.join(args.workspace, args.output), exist_ok=True)
    labels = read_labels(args.workspace)  # 读取标签
    colors = colorize_labels(labels)  # 读取颜色
    mesh = colorize_mesh(mesh, colors)  # 颜色化mesh
    save_mesh(mesh, os.path.join(args.workspace, args.output)) # 保存mesh
